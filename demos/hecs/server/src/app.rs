...
impl App {
    pub async fn new(server_config: ServerConfig) -> Self {
        let mut server =
            Server::new(manifest_load(), Some(server_config), get_shared_config()).await;
...

        {
            ...
            .iter()
            {
                ...
                let _pos_key = server
                    .add_component_to_entity(&naia_key, Components::Position(position.clone()));

                // Add name component to Entity
                let name = Name::new(first, last).wrap();
                let _name_key =
                    server.add_component_to_entity(&naia_key, Components::Name(name.clone()));

                // Add to World
                let hecs_key = world.spawn((Ref::clone(&name), Ref::clone(&position)));
    ...
    fn remove_component<T: 'static + Replicate<Components>>(
        &mut self,
