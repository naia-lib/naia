...
fn apply_pending_scope_changes_drains_entity_enter_room_variant() {
    ...
    let pre_add_len = queue_len(&coord);
    coord.room_add_entity(&room_key, &entity);
    assert!(
        queue_len(&coord) >= pre_add_len + 2,
...
fn apply_pending_scope_changes_is_idempotent_within_one_tick() {
    ...
    coord.room_add_entity(&k2, &entity);
    let post_push_len = queue_len(&coord);
...
fn user_left_room_variant_is_drained() {
    ...
    coord.room_remove_user(&room_key, &synthetic_user_key);
    assert!(
        queue_len(&coord) >= 4,
