...
fn sim_enable_matches_legacy_observables() {
    ...
    assert_eq!(from_sim.0, ReplicationConfig::public());
    assert_eq!(from_sim.0.scope_exit, ScopeExit::Despawn);
    assert!(matches!(from_sim.1, EntityOwner::Server));
}

#[test]
fn sim_enable_composes_with_configure_in_same_tick() {
    ...
    fn run_with(sim_enable: bool) -> ReplicationConfig {
        ...
        let (sim_handle, recv, send) = if sim_enable {
            sim_handle.enable_entity_replication(&entity);
            (sim_handle, recv, send)
        } else {
            let (sim_handle, recv, send, ()) =
    ...
    assert_eq!(from_sim.scope_exit, ScopeExit::Persist);
}
