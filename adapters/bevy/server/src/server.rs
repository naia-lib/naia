...
impl ServerImpl {
    ...
    pub(crate) fn enable_static_entity_replication(&mut self, entity: &Entity) {
        match self {
            Self::Full(server) => server.enable_static_entity_replication(entity),
            Self::WorldOnly(server) => server.enable_static_entity_replication(entity),
        }
    }
