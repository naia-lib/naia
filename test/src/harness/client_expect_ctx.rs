...
impl<'b, 'a: 'b> ClientExpectCtx<'b, 'a> {
    ...
    pub fn has_entity(&self, entity: &EntityKey) -> bool {
        ...
        self.expect_ctx.scenario.client_entity_ref(&self.client_key, &user_key, entity).is_some()
    }
    ...
    pub fn entity(&'_ mut self, entity: &EntityKey) -> Option<EntityRef<'_, TestEntity, WorldRef<'_>>> {
        let user_key = self.expect_ctx.scenario.user_key(&self.client_key);
        self.expect_ctx.scenario.client_entity_ref(&self.client_key, &user_key, entity)
    }
