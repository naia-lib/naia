...
impl<'b, 'a: 'b> ClientExpectCtx<'b, 'a> {
    pub(crate) fn new(expect_ctx: &'b mut ExpectCtx<'a>, client_key: ClientKey) -> Self {
        Self { expect_ctx, client_key }
    }

    pub fn has_entity(&mut self, entity: &EntityKey) -> bool {
        let user_key = self.expect_ctx.scenario().user_key(&self.client_key);
        self.expect_ctx.scenario_mut().client_entity_ref(&self.client_key, &user_key, entity).is_some()
    }
    ...
    pub fn entity(&'_ mut self, entity: &EntityKey) -> Option<EntityRef<'_, TestEntity, WorldRef<'_>>> {
        let user_key = self.expect_ctx.scenario().user_key(&self.client_key);
        self.expect_ctx.scenario_mut().client_entity_ref(&self.client_key, &user_key, entity)
    }
