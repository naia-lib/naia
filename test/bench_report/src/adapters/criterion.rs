...
use crate::core::model::{BenchResult, CriterionMessage};
use crate::ports::source::BenchResultSource;

/// Reads `cargo criterion --message-format=json` from stdin.
pub struct CriterionSource;

impl BenchResultSource for CriterionSource {
...
fn extract_result(msg: CriterionMessage) -> Option<BenchResult> {
    ...
    let throughput_unit = msg.throughput.as_ref()
        .and_then(|v| v.first())
        .map(|t| t.unit.clone());

    let throughput_per_iter = msg.throughput.as_ref()
        .and_then(|v| v.first())
    ...
    Some(BenchResult { id, category, sub_id, param, median_ns, std_dev_ns, throughput_unit, throughput_per_iter })
}
...
mod tests {
    use super::*;
    use crate::core::model::CriterionMessage;
