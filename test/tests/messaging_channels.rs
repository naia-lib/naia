...
use naia_test::test_protocol::{
    ...
    ReliableChannel, UnreliableChannel, OrderedChannel, UnorderedChannel,
    SequencedChannel, TickBufferedChannel, RequestResponseChannel,
};
...
fn reliable_server_to_clients_broadcast_respects_rooms() {
    ...
    let room_r_key = make_room(&mut scenario);
    let room_s_key = make_room(&mut scenario);

    let client_a_key = client_connect(&mut scenario, &room_r_key, "Client A", Auth::new("client_a", "password"), test_protocol.clone());
    let client_b_key = client_connect(&mut scenario, &room_r_key, "Client B", Auth::new("client_b", "password"), test_protocol.clone());
    let client_c_key = client_connect(&mut scenario, &room_s_key, "Client C", Auth::new("client_c", "password"), test_protocol);
    ...
    scenario.expect(|ctx| {
        ...
        for msg in ctx.client(client_a_key, |c| c.read_message::<ReliableChannel, TestMessage>()) {
            if msg.value == 42 {
                a_received = true;
            }
        }
        for msg in ctx.client(client_b_key, |c| c.read_message::<ReliableChannel, TestMessage>()) {
            if msg.value == 42 {
                b_received = true;
            }
        }
        for msg in ctx.client(client_c_key, |c| c.read_message::<ReliableChannel, TestMessage>()) {
            if msg.value == 42 {
                c_received = true;
            }
        }
...
fn reliable_point_to_point_request_response() {
    ...
    let response_key = scenario.mutate(|ctx| {
        ctx.client(client_a_key, |client_a| {
            client_a.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new("query"))
    ...
    let response_id = scenario.expect(|ctx| {
        ctx.server(|server| {
            // Read request from A
            for (client_key, response_id, request) in server.read_request::<RequestResponseChannel, TestRequest>() {
...
fn per_channel_ordering() {
    ...
    scenario.expect(|ctx| {
        ...
        let a_unordered: Vec<u32> = ctx.client(client_a_key, |c| {
            c.read_message::<UnorderedChannel, TestMessage>().map(|m| m.value).collect()
        });
        ...
        (a_ordered_correct && b_ordered_correct && a_unordered_has_both && b_unordered_has_both).then_some(())
    });
}
...
fn ordered_reliable_channel_keeps_order_under_latency_and_reordering() {
    ...
    scenario.mutate(|ctx| {
        ctx.server(|server| {
            ...
            server.send_message::<OrderedChannel, _>(&client_a_key, &TestMessage::new(3)); // C
        });
    });
...
fn unordered_reliable_channel_delivers_all_messages_but_in_arbitrary_order() {
    ...
    scenario.mutate(|ctx| {
        ctx.server(|server| {
            ...
            server.send_message::<UnorderedChannel, _>(&client_a_key, &TestMessage::new(3)); // C
        });
    });
...
fn unordered_unreliable_channel_shows_best_effort_semantics() {
    ...
    scenario.expect(|ctx| {
        let messages: Vec<u32> = ctx.client(client_a_key, |c| {
            c.read_message::<UnreliableChannel, TestMessage>().map(|m| m.value).collect()
        });
...
fn sequenced_reliable_channel_only_exposes_the_latest_message_in_a_stream() {
    ...
    scenario.mutate(|ctx| {
        ctx.server(|server| {
            ...
            server.send_message::<SequencedChannel, _>(&client_a_key, &TestMessage::new(3)); // S3
        });
    });
    ...
    scenario.expect(|ctx| {
        let messages: Vec<u32> = ctx.client(client_a_key, |c| {
            c.read_message::<SequencedChannel, TestMessage>().map(|m| m.value).collect()
        });
...
fn server_to_client_request_yields_exactly_one_response() {
    ...
    let response_key = scenario.mutate(|ctx| {
        ctx.server(|server| {
            server.send_request::<RequestResponseChannel, TestRequest>(&client_a_key, &TestRequest::new("query"))
                .expect("Failed to send request")
        })
    });
...
fn many_concurrent_requests_from_a_single_client_remain_distinct() {
    ...
    let response_keys: Vec<_> = scenario.mutate(|ctx| {
        ctx.client(client_a_key, |client_a| {
            (0..5).map(|i| {
                client_a.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new(&format!("query_{}", i)))
                    .expect("Failed to send request")
            }).collect()
        })
    });
    ...
    let response_ids: Vec<(naia_shared::GlobalResponseId, String)> = scenario.expect(|ctx| {
        ctx.server(|server| {
            ...
            if ids.len() == 5 { Some(ids) } else { None }
        })
    });
...
fn concurrent_requests_from_multiple_clients_stay_isolated_per_client() {
    ...
    let response_key_b = scenario.mutate(|ctx| {
        ctx.client(client_b_key, |client_b| {
            client_b.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new("query"))
                .expect("Failed to send request")
        })
    });
    ...
    let response_ids: Vec<(ClientKey, naia_shared::GlobalResponseId)> = scenario.expect(|ctx| {
        ctx.server(|server| {
            let mut ids = Vec::new();
            for (client_key, response_id, _request) in server.read_request::<RequestResponseChannel, TestRequest>() {
...
fn response_completion_order_is_well_defined_and_documented() {
    ...
    let response_keys: Vec<_> = scenario.mutate(|ctx| {
        ctx.client(client_a_key, |client_a| {
            vec![
                client_a.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new("1")).expect("Failed"),
                client_a.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new("2")).expect("Failed"),
                client_a.send_request::<RequestResponseChannel, TestRequest>(&TestRequest::new("3")).expect("Failed"),
            ]
        })
    });
