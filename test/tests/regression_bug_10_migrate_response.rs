...
fn bug_10_remote_entity_lookup_fails() {
    ...
    assert!(found_remote.is_err(), "RemoteEntity doesn't exist yet - this is the bug!");
}
...
fn bug_10_server_sends_wrong_entity_ids() {
    ...
    println!("Client: Created entity as HostEntity({:?})", client_host_entity);
    ...
    assert!(lookup_result.is_err(), "Client can't find server's RemoteEntity ID!");
    ...
    assert!(correct_lookup.is_ok(), "Client CAN find its own HostEntity!");
}
