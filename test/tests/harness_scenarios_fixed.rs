...
fn harness_delegation_flow_smoke() {
    ...
    scenario.expect(|ctx| {
        ctx.server(|sv| {
            ...
            c.entity(ent).replication_is_delegated() && c.entity(ent).auth_is(EntityAuthStatus::Granted)
        })
    });
    ...
    scenario.expect(|ctx| {
        ctx.client(b, |c| {
            c.sees(ent) && c.entity(ent).replication_is_delegated() && c.entity(ent).auth_is(EntityAuthStatus::Available)
        })
    });
