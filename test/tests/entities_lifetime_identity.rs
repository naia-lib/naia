...
fn component_insertion_after_initial_spawn() {
    ...
    let client_c_key = client_connect(&mut scenario, &room_key, "Client C", Auth::new("client_c", "password"), test_protocol);
    ...
    scenario.expect(|ctx| {
        ctx.client(client_c_key, |c| c.has_entity(&entity_e)).then_some(())
    });
}
...
fn long_running_connect_disconnect_and_spawn_despawn_cycles_do_not_leak() {
    ...
    for i in 0..3 {
        let client_key = client_connect(&mut scenario, &room_key, &format!("Client {}", i), Auth::new(&format!("client_{}", i), "password"), test_protocol.clone());
        ...
        for j in 0..2 {
            ...
            scenario.mutate(|ctx| {
                ctx.server(|server| {
                    server.entity_mut(&entity).unwrap().despawn();
                });
            });
