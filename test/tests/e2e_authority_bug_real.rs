...
use naia_server::{
    DelegateEntityEvent, EntityAuthGrantEvent as ServerEntityAuthGrantEvent,
    Server as NaiaServer, ServerConfig,
};
use naia_shared::WorldRefType;
use naia_test::{
    local_socket_pair, protocol, Auth, Position, TestEntity, TestWorld,
    update_client_server, run_updates, wait_for_delegation_event,
    wait_for_authority_status, wait_for_condition, complete_handshake,
...
fn log_server_events(server: &mut Server, step: &str) {
    ...
    let mut auth_grant_count = 0;
        for _ in server_events.read::<ServerEntityAuthGrantEvent>() {
        auth_grant_count += 1;
    }
