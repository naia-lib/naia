...
use naia_shared::{serde::{BitReader, BitWriter, Serde}, Protocolize, StandardHeader, PacketType};
...
fn end_to_end_handshake_w_auth() {
    ...
    // 6. Server receive connect request
    {
        ...
        if let HandshakeResult::AuthUser(auth_message) = result {
            ...
            assert_eq!(*auth_replica.username, username, "Server received an invalid username: '{}', should be: '{}'", *auth_replica.username, username);
            assert_eq!(*auth_replica.password, password, "Server received an invalid password: '{}', should be: '{}'", *auth_replica.password, password);
        } else {
