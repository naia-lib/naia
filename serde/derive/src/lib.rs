...
pub fn derive_ser(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let ts = match &input {
        parse::Data::Struct(struct_) if struct_.named => derive_ser_struct(struct_),
        parse::Data::Struct(struct_) => derive_ser_struct_unnamed(struct_),
...
pub fn derive_de(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // ok we have an ident, its either a struct or a enum
    let ts = match &input {
        parse::Data::Struct(struct_) if struct_.named => derive_de_struct(struct_),
        parse::Data::Struct(struct_) => derive_de_struct_unnamed(struct_),
