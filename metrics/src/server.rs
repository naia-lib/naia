...
pub fn emit_server_connection_stats(stats: &ConnectionStats, user_id: u64) {
    ...
    metrics::gauge!(names::SERVER_CONN_RTT_MS,      "user_id" => id.clone())
        .set(stats.rtt_ms as f64);
    metrics::gauge!(names::SERVER_CONN_RTT_P99_MS,  "user_id" => id.clone())
        .set(stats.rtt_p99_ms as f64);
    metrics::gauge!(names::SERVER_CONN_JITTER_MS,   "user_id" => id.clone())
        .set(stats.jitter_ms as f64);
    metrics::gauge!(names::SERVER_CONN_PACKET_LOSS, "user_id" => id.clone())
        .set(stats.packet_loss_pct as f64);
    metrics::gauge!(names::SERVER_CONN_KBPS_SENT,   "user_id" => id.clone())
        .set(stats.kbps_sent as f64);
