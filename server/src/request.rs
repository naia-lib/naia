...
impl GlobalResponseManager {
    ...
    pub(crate) fn create_response_id(&mut self, user_key: &UserKey, channel_kind: &ChannelKind, local_response_id: &LocalResponseId) -> GlobalResponseId {
        ...
        self.map.insert(id, (user_key.clone(), channel_kind.clone(), local_response_id.clone()));

        id
    }

    pub(crate) fn destroy_response_id(&mut self, global_response_id: &GlobalResponseId) -> Option<(UserKey, ChannelKind, LocalResponseId)> {
