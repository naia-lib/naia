//! Pipeline-mode handles for the recv and send halves of a `InternalWorldServer`
//! (step 4-E.2f).
//!
...
//! and `InternalWorldServer::from_pipeline_states(...)` reassembles them. The
...
//! `into_pipeline_handles` — it keeps `InternalWorldServer` reassembled and
...
//! `InternalWorldServer::process_all_packets`, which mutates `self.send.*`
//! while doing so. After `into_pipeline_handles`, the coordination side no longer
//! holds a unified `InternalWorldServer`. The .e MVP works around this by NOT
...
//! `InternalWorldServer::send_all_packets` for the three-phase factoring plan.
//!
...
impl<E: Copy + Eq + Hash + Send + Sync> RecvHandle<E> {
    ...
    ///   1. `InternalWorldServer::drain_pending_handshakes` (coordination-stage; needs
    ...
    /// (mirroring `InternalWorldServer::finalize_connection`) and inject one
...
impl<E: Copy + Eq + Hash + Send + Sync> SendHandle<E> {
    ...
    /// the Sim/main thread has called `InternalWorldServer::run_send_preamble` on
    ...
    // reassembles a `InternalWorldServer`. Thin forwards to the `SendState`
    // namesakes (which mirror `InternalWorldServer::*` field-for-field — see the
    // D.3b.2 section in `send_state.rs`). Existing `InternalWorldServer::*` methods
    // are unchanged.
    // ====================================================================
    ...
    /// `InternalWorldServer::insert_component_worldless`. Mutates `shared.gwm` +
    ...
    /// `InternalWorldServer::remove_component_worldless`.
    ...
    /// `InternalWorldServer::despawn_entity_worldless`. Takes `&mut CoordinatorState`
    ...
    // `InternalWorldServer::user_scope_mut` chain that depends on
    ...
    ///
    /// Mirrors `InternalWorldServer::scope_checks_pending()` but operates against
    ...
    /// `InternalWorldServer::user_scope_set_entity` runs — D5's send-side scope
    ...
    // ====================================================================
    //
    // Mirrors `InternalWorldServer::user_scope_has_entity` (world_server.rs:2292)
    ...
    ///
    /// Decision logic (byte-identical to `InternalWorldServer::user_scope_has_entity`
