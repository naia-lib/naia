...
//!   * 4-E.2e — `global_priority` → `SendState` (deferred from 4-E.2d;
//!     dual-write through the `EntityPriorityMut` borrow API would have
//!     required touching the shared crate's public types, so the field
//!     move is bundled with `SendStateUpdate::PriorityChanged` instead)
//!
...
pub struct CoordinatorState<E: Copy + Eq + Hash + Send + Sync> {
    ...
    /// Sender-wide priority layer. → SendState (4-E.2e, deferred from 4-E.2d).
