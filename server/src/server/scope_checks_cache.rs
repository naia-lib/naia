...
/// `InternalWorldServer::scope_checks()`. Replaces the per-tick
...
impl<E: Copy + Eq + Hash + Send + Sync> ScopeChecksCache<E> {
    ...
    /// rooms — invoked by `InternalWorldServer::despawn_entity_worldless` so the
...
mod tests {
    ...
    //! debug-build assertion in `InternalWorldServer::scope_checks()` (every 1024th
    ...
    /// `InternalWorldServer::scope_checks_recompute_slow` for these tests.
    ...
    fn world_server_room_ops_observe_same_state_post_refactor() {
        // Regression: InternalWorldServer::room_add_user should produce the same cache
