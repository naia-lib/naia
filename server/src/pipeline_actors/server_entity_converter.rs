...
//! a `InternalWorldServer<E>`.
//!
//! # Why this exists
//!
//! Today the only `EntityAndGlobalEntityConverter<E>` implementer is
//! `InternalWorldServer<E>` (`server/src/server/world_server.rs:3956`), whose
...
//! `InternalWorldServer`'s converter reads, `EntityProperty::set` produces a
