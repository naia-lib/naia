...
/// An Enum with a variant for every Object/Component/Message that can be sent between
/// Client/Host
pub trait ProtocolType<Impl = Self>: Clone {
    ...
    /// Convert ProtocolType to an inner reference of the Object/Component/Message
    fn inner_ref(&self) -> Ref<dyn Replicate<Impl>>;
    /// Compare properties in another ProtocolType
    fn equals(&self, other: &Impl) -> bool;
    /// Sets the current Object/Component/Message to the state of another of the same type
