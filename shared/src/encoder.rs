...
impl DictionaryTrainer {
    /// `target_sample_size` here describes the number of samples (packets) to train on.
    ...
    pub fn record_bytes(&mut self, bytes: &[u8]) {
        ...
        if current_sample_size >= self.next_alert_size {
            let percent = ((self.next_alert_size as f32) / (self.target_sample_size as f32)) * 100.0;
        ...
        if current_sample_size >= self.target_sample_size {
            ...
            let dictionary = from_continuous(&self.sample_data, &self.sample_sizes, target_dict_size).expect("Error while training dictionary");
