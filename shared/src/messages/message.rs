...
impl Messages {
    ...
    pub fn read(bit_reader: &mut BitReader, converter: &dyn NetEntityHandleConverter) -> Result<Box<dyn Message>, SerdeErr> {
        todo!()
    }

    pub fn write<M: Message>(bit_writer: &mut dyn BitWrite, converter: &dyn NetEntityHandleConverter, message: &M) {
        todo!()
    }
}
