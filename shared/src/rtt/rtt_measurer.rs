use crate::rtt::rtt_data::RttData;
...
impl RttMeasurer {
    ...
    /// This will get the smoothed round trip time (rtt) from the time we last
    /// heard from a packet.
    fn get_smoothed_rtt(&self, rtt_entry: Option<&mut RttData>) -> f32 {
        match rtt_entry {
            Some(avoidance_data) => {
                let rtt_time = avoidance_data.sending_time.elapsed().as_millis();
    ...
    /// Smooths out round trip time (rtt) value by the specified smoothing
    /// factor.
    ///
    ...
    ///
    /// We do this so that if one packet has an bad rtt it will not directly
    /// bring down the or network quality estimation. The default is 10%
    /// smoothing so if in total or packet is 50 milliseconds later than max
    /// allowed rtt we will increase or rtt estimation with 5.
    fn smooth_out_rtt(&self, rtt: u128) -> f32 {
