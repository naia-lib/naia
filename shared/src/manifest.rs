...
use super::{impls::ProtocolType, protocol_type::ProtocolKindType, replica_builder::ReplicaBuilder};
...
impl<P: ProtocolType> Manifest<P> {
    ...
    pub fn register_replica(&mut self, replica_builder: Box<dyn ReplicaBuilder<P>>) {
        self.builder_map.insert(replica_builder.get_kind(), replica_builder);
    }

    /// Creates a Message/Component instance, given a NaiaId and a
    /// payload, typically from an incoming packet
    pub fn create_replica(&self, component_kind: P::Kind, reader: &mut PacketReader, packet_index: u16) -> P {
        ...
        panic!("No ReplicaBuilder registered for NaiaId: {}", component_kind.to_u16());
    }
}
