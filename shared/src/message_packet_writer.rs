...
impl MessagePacketWriter {
    ...
    pub fn bytes(&mut self, out_bytes: &mut Vec<u8>) {
        ...
        if self.message_count != 0 {
            ...
            // write number of messages in the following message
            out_bytes.write_u8(self.message_count).unwrap();

            // write message payload
            out_bytes.append(&mut self.message_working_bytes);

            self.message_count = 0;
            self.message_working_bytes = Vec::<u8>::new();
        }
    }

    /// Returns whether or not the given message will fit in the outgoing buffer
    pub fn message_fits<P: Protocolize>(&self, total_bytes: usize, message: &P) -> bool {
