...
/// Add additional trait dependencies to the ReplicateSafe trait definition
/// comma separated
macro_rules! define_trait_ReplicateSafe {
    ($($p:path), *) => {
        /// The part of ReplicateSafe which is object-safe
        pub trait ReplicateSafe<P: ProtocolType>: $( $p + )* Sync + Send + 'static {
...
// Add a requirement for bevy Component to be implemented
#[cfg(feature = "bevy-support")]
define_trait_ReplicateSafe! {
    Component<Storage = TableStorage>
}

#[cfg(not(feature = "bevy-support"))]
define_trait_ReplicateSafe! {}
