    ...
    for (index, variant) in enum_.variants.iter().enumerate() {
        ...
        // Struct Variant
            ...
            for field in &variant.fields {
                l!(ser_variants, "{}, ", field.field_name.as_ref().expect("expected the field to have a name"));
            }
            l!(ser_variants, "} => {");
            ...
            for field in &variant.fields {
                l!(
                    ser_variants,
                    "{}.ser(writer);",
                    field.field_name.as_ref().expect("expected the field to have a name")
                );
            }
            l!(ser_variants, "}");
        }
        ...
        // Struct Variant
            ...
            for field in &variant.fields {
                l!(
                    de_variants,
                    "{}: Serde::de(reader)?,",
                    field.field_name.as_ref().expect("expected field to have a name")
                );
            }
            l!(de_variants, "},");
        }
