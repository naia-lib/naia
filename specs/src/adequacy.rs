...
pub fn run_adequacy(index: &Index, strict: bool) -> Result<()> {
    ...
    for cid in contracts_sorted {
        ...
        if let Some(obligations) = index.contract_obligations.get(cid) {
            ...
            for obl in obligations {
                // Check if any label matches exact format "cid: obl" or "cid.obl"
                // Spec tool might be strict about separator.
                // Assuming standard "cid: obl" or "cid.obl"
                let target1 = format!("{}: {}", cid, obl);
                let target2 = format!("{}.{}", cid, obl);
                if !found.contains(&target1) && !found.contains(&target2) {
                     missing.push(obl);
                }
            }
        }
    ...
    if missing_obligations_count > 0 {
        ...
        for (cid, missing) in missing_obligations {
             ...
             println!("     Next: ./spec_tool.sh packet {}", cid);
             println!();
        }
    }
