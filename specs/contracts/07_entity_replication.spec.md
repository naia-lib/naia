...
- RPC/message semantics (see `03_messaging.spec.md`)
...
- **Entity lifetime (client-side)**: `scope enter → scope leave`, where re-entering scope after being out-of-scope for **≥ 1 tick** is a **new lifetime** (fresh spawn semantics). See `06_entity_scopes.spec.md`.
...
**Obligations:**
- **t1**: Global identity stability works correctly
...
**Obligations:**
- **t1**: Client-visible lifetime boundaries works correctly
...
Cross-link:
- Scope/lifetime rules are defined in `06_entity_scopes.spec.md` and are binding here.
...
**Obligations:**
- **t1**: Spawn snapshot semantics (baseline state) works correctly
...
**Obligations:**
- **t1**: No observable replication before Spawn works correctly
...
**Obligations:**
- **t1**: Actions outside lifetime are ignored works correctly
...
**Obligations:**
- **t1**: Update-before-Insert buffering (within lifetime) works correctly
...
**Obligations:**
- **t1**: Local-only component overwrite by server replication works correctly
...
Cross-link:
- Ownership rules for local-only components vs server-backed replicated components are defined in `08_entity_ownership.spec.md`. This contract ensures replication behavior conforms.
...
**Obligations:**
- **t1**: Collapse to final state per tick (no intermediate transitions) works correctly
...
**Obligations:**
- **t1**: Duplicate delivery is idempotent works correctly
...
**Obligations:**
- **t1**: Identity reuse safety (LocalEntity wrap/reuse) works correctly
...
**Obligations:**
- **t1**: GlobalEntity rollover is a terminal error works correctly
...
**Obligations:**
- **t1**: Conflict resolution: server wins for replicated state works correctly
...
- Delegated authority refinement:
    ...
    - If the server revokes/resets authority, the canonical source may transition back to server-originated state after the reset boundary (see `11_entity_authority.spec.md`).
...
- `06_entity_scopes.spec.md` — defines scope enter/leave semantics and the ≥1 tick lifetime rule
- `09_entity_publication.spec.md` — defines publish/unpublish interactions with scope
- `08_entity_ownership.spec.md` — defines local-only mutation rules and ownership write constraints
- `10_entity_delegation.spec.md` / `11_entity_authority.spec.md` — define delegation and authority semantics
- `13_client_events_api.spec.md` — defines client-observable event ordering/meaning
- `04_time_ticks_commands.spec.md` — defines tick semantics (including wrap considerations)