> **ARCHIVED 2026-06-30 — Missions 0 and 1 below are COMPLETE.** Kept as historical
> record; do not follow as forward instructions. The live handoff doc
> (`_AGENTS/MISSION_HANDOFF_NAIA_SESSION.md`) now covers only Mission 2.
>
> - **Mission 0** (coupled re-insert fix) landed ~2026-06-29 08:52 — naia-side fix
>   (`shared/src/world/host/host_world_manager.rs`, `delivered_component_kinds`)
>   is still in place. The cyberlith-side Layer-1 guard
>   (`services/game/cell/src/resource_replication.rs`) was later **superseded
>   and deleted entirely** by cyberlith `7a8fc2a9a` ("G6: replicated resources
>   via naia pipeline_* API; delete carrier glue", 2026-06-30), which moved the
>   pipelined-sim resource-replication machinery into naia proper
>   (`adapters/bevy/server/src/server.rs`, naia `286bdbab`) per the boundary
>   mission. That file no longer exists in cyberlith.
> - **Mission 1** (test/bench/tools reorg) landed via naia `6ce04cc4` ("project:
>   reorg test/ bench/ tools/ structure", 2026-06-29 09:15). The target
>   structure described below exists at `test/`, `bench/`, `tools/spec_tool/`.
>
> Mission 2 (sim-pipelined namako BDD coverage) was still open as of archiving
> and is carried forward in the live doc.
