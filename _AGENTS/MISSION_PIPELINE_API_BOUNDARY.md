...
- ❌ **NOT proven by the spike:** the **production needed-set `SnapshotWorld` assembler** that G7 §2g-item-4 introduces. The spike hand-builds the snapshot via `insert_component`; it does NOT exercise the core `WorldRefType` assembler (the code whose panic-vs-skip is M2 and whose perf is M3). **G7 acceptance obligation:** add a byte-identity test that drives the *real* core assembler (once it exists) under a multi-tick run with mutation between freeze and transmit, against the resident baseline. Until then, assembler fidelity is asserted (via the copy_to_box chain trace, §2g item 4), not empirically shown. This is explicitly a G7 gate, not a closed item.
