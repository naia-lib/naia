...
> This is the reframed "enforcement Item 2" + the "more to sweep" Connor flagged.
> **P1/P2/P3/P6 ✅ COMPLETE (2026-06-30) — verified against `git log dev` + the cyberlith/diax
> companion repos, not transcribed:**
> - **P1** — naia dev `98d39a6b` ("G-unify P1: complete WorldServer unified surface + NaiaServer
>   holds it"): grew `WorldServer<E>`'s surface (the ~31-method diff below), swapped
>   `NaiaServer.world_server: InternalWorldServer<E>` → `WorldServer<E>`, added `Server::new_pipelined`.
> - **P3 + P6** — naia dev `1935f6d1` ("G-unify P2/P3/P6: delete as_pipelined*, first-class coord
>   ops"): de-downcasted the bevy adapter (`apply_receive_output.rs`, `host_sync_pipeline.rs`,
>   `plugin_full.rs`, `server.rs`, the 5 `sim_integration_full_*` tests) onto the unified
>   `WorldServer` surface, and **DELETED `WorldServer::as_pipelined`/`as_pipelined_mut` outright**
>   (confirmed: `grep -rn "as_pipelined" --include="*.rs" .` in this repo now matches only stale
>   doc-comments referencing the old name, no method definitions or call sites).
> - **P2** — landed as a same-day companion commit in the **cyberlith** repo (main `4b0c61800`,
>   "game/cell: migrate off deleted naia as_pipelined\* to first-class WorldServer ops"): the 8
>   `as_pipelined_mut()` reach sites in `services/game/cell` (`server_access.rs`,
>   `asset/asset_scope_machine.rs`, `systems/startup.rs`) now call `&mut WorldServer<Entity>`
>   directly; `cybertool check naia-primitive-reach` FLOOR_BASELINE lowered accordingly. A
>   companion **diax** commit (main `fce3437`) updated `handle_scope_actions` to take
>   `&mut WorldServer` instead of `&mut CoordHandle`. (1935f6d1's own commit message flagged
>   cyberlith P2 + the diax change as "still pending — land together with this"; both landed
>   shortly after, confirmed present on cyberlith `main`/diax `main` HEAD at time of writing.)
> - Follow-up naia dev `59f4b25d` ("add first-class WorldServer::disconnect_user") was needed
>   because cyberlith's out-of-band cell-disconnect path had reached `ps.disconnect_user` via the
>   now-deleted `as_pipelined_mut` — not one of the 6 named phases, but required to keep P6 from
>   stranding that call site.
>
> **REMAINING — pick up here: P4 + P5 only.** P4 (harness `Scenario::new(ServerMode)`) — verified
> count: `grep -rln "Scenario::new(" test/` → exactly **132 call sites across 15 files** (re-counted
> fresh, matches the figure already in the P4 bullet below). P5 (the real-ack byte-identity test) —
> verified the target file does not exist yet: only
> `test/harness/contract_tests/integration_only/g9pre_resident_pipelined_byte_identity.rs` is
> present (the settle-to-silence sidestep P5 exists to close);
> `g8_real_ack_byte_identity.rs` is unwritten. Start with P4 (it's a mechanical
> `Scenario::new()` → `Scenario::new(ServerMode::Resident)` sweep + `server_start` dispatch), then
...
- **P1 ✅ DONE (naia dev `98d39a6b`) — (naia engine — the bulk):** grow `WorldServer`'s unified surface to cover EVERY method
...
- **P2 ✅ DONE (cyberlith main `4b0c61800`; companion diax main `fce3437`) — (cyberlith):** de-downcast the 8 `as_pipelined_mut()` floor reaches in services/game/cell
...
- **P3 ✅ DONE (naia dev `1935f6d1`) — (naia bevy adapter):** de-downcast the 10 reaches in `adapters/bevy/server/src/`
  (server.rs, plugin_full.rs, host_sync_pipeline.rs) onto the unified surface.
- **P4 — REMAINING, pick up here (harness):** `Scenario::new(mode: ServerMode)` — Connor wants it REQUIRED/explicit (no default);
  ~132 `Scenario::new()` callers across 15 files (re-verified 2026-06-30: `grep -rln "Scenario::new(" test/`
  → 132 occurrences / 15 files, exact) → `Scenario::new(ServerMode::Resident)`. `server_start`
  builds NaiaServer resident vs pipelined per stored mode. ServerMut/MutateCtx machinery unchanged (still
  NaiaServer).
- **P5 — REMAINING (the test):** `test/harness/contract_tests/integration_only/g8_real_ack_byte_identity.rs` (verified 2026-06-30: file does not exist yet) — ONE
...
- **P6 ✅ DONE (naia dev `1935f6d1`, same commit as P3) — (cleanup):** delete `WorldServer::as_pipelined`/`as_pipelined_mut` once no consumer downcasts. Verified deleted: `grep -rn "as_pipelined" --include="*.rs" .` only matches stale doc-comments, no live method/call sites.
...
| G3b | cyberlith D11 `CellCommandsExt` dies, replaced by direct `pipelined_server.method()` calls in the park window | **PARTIAL (re-verified 2026-06-30)** — the downcast this targeted is gone: cyberlith `main 4b0c61800` migrated `CellCommandsExt`'s own methods to take `&mut WorldServer<Entity>` directly (no more `as_pipelined_mut()`). But the trait itself was NOT deleted — still defined `services/game/cell/src/server_access.rs:824` and still the call idiom at `asset/asset_scope_machine.rs:82` (`commands.spawn_empty().enable_replication(ws)`). Full retirement of `CellCommandsExt` is outside FINISH-G-UNIFY P1–P6's scope and remains open. |
| G3c | unified `Server` param: add `ServerImpl::Pipelined(PipelinedServer<Entity>)` variant; **NO panic arms** — all mutating methods (incl. `send_message`/`broadcast_message`/`take_authority`) work from any system via internal queueing (§2e); reads return coord state; retire raw `ResMut<PipelinedServer>` access | **SUPERSEDED** — §2n (line ~533) states this explicitly: "This SUPERSEDES the old G3c (...)"; the design moved to unifying at the `WorldServer` enum instead of adding an adapter-level `ServerImpl::Pipelined` variant (§2n: "the bevy adapter's `ServerImpl` needs NO `Pipelined` variant"). Re-verified 2026-06-30: `grep -rn "ServerImpl::Pipelined\|ResMut<PipelinedServer>\|struct PipelinedServer" adapters/bevy/server/src/` is empty — no such variant/Resource exists. The "retire raw `ResMut<PipelinedServer>`" half is DONE via §2o Phase 5c-2 (naia dev `32efd38d`, "remove PipelinedServer resource; drive pipeline through ServerImpl"). |
...
| G5b | **(per §2h C2 — editor/delegated path IN SCOPE, Connor 2026-06-29)** queued `entity_take_authority` (+ related authority ops), drained at the editor-ops phase | **DONE 2026-06-30** — naia dev `6d8b80e9` ("relocate sim-side room/authority op drain into naia engine": `PipelineScopeAuthorityOp`/`PendingScopeAuthorityOps` + `Server::pipeline_drain_scope_authority_ops`); companion consumer-side cleanup landed as **cyberlith** main `973e326a5` ("collapse editor-op drain onto naia's pipeline_drain_scope_authority_ops") — NOT a naia-repo SHA, verified `973e326a5` does not exist in `naia` (`git cat-file -t` fails here; it resolves only in the cyberlith checkout). Matches the frontmatter's own "S6 ergonomic collapse COMPLETE" line, which already cited `6d8b80e9` for G5b. |
| G6 | `Res<R>` resource API (`SimPipeline::insert_resource` etc.) | **DONE 2026-06-30** — naia dev `286bdbab` ("pipeline replicated-resource API (Res<R>) for sim mode": `pipeline_replicate_resource`/`pipeline_remove_replicated_resource`/`pipeline_drain_resource_registrations`). Matches the frontmatter's own "S6 ergonomic collapse COMPLETE" line. |
| G6b | **(per §2h H2 + audit N3 — OPEN, design pass required)** host-sync drain (bevy change-detection → replication config). This is the **least framework-agnostic** piece in the mission: `drain_sim_host_sync_pipelined` (`server_access.rs:275-320`) is driven by bevy `Messages<HostSyncEvent>` against the **Sim** world. Open forks: (a) can a non-bevy consumer even have host-sync, or is it a bevy-adapter-only convenience? (b) does it survive in a no-reassembly core (§6 forbids reassembly post-G10)? (c) can every `HostSyncEvent` producer be retired by explicit G4/G5 `spawn_replicated`/`enable_replication`, removing the bridge entirely? **Decide before G10.** | **DONE 2026-06-30** — naia dev `00f09b2a` ("pipeline_drain_host_sync — relocate host-sync drain into the engine": the take_handles→drain→restore_handles wrapper moved into `Server::pipeline_drain_host_sync`, panic-safe unconditional restore). Companion consumer-side cleanup landed as cyberlith main `00b2c0aa7` ("G6b: drain host-sync via naia pipeline_drain_host_sync; zero take_handles"). Matches the frontmatter's own "S6 ergonomic collapse COMPLETE" line, which already cited `00f09b2a` for G6b. |
...
| G9 | **`ServerMode::{Resident, Pipelined}` — single knob** — Pipelined⇒worker send, Resident⇒synchronous send. Same `receive`/`send` signatures; consumer code unchanged. **Oracle = synchronously-driven Pipelined bracket (NOT Resident)** — per §2i, identical bytes to production by construction; **no `SendStrategy` knob needed**, single-knob purity holds. | **REALIZED — DONE, via §2n/§2o** (re-verified 2026-06-30; the row's old status text was self-contradictory — it cited "design §2f — pending sign-off" while §2f's own header already reads "✅ SIGNED OFF + IMPLEMENTED 2026-06-29"). G9's exact ask (`ServerMode` single knob, same `receive`/`send` signatures, consumer code unchanged) is what §2n's unified `WorldServer` enum + §2o's phased build delivered: `ServerMode::{Resident, Pipelined}` + the `WorldServer` enum shell landed in §2o Phase 2c-2 (naia dev `755e1152`); the unified `receive`/`send` dispatch with byte-identical signatures across both variants landed in §2o Phase 3 (naia dev `7845db4d`) — both ✅ in §2o, both confirmed present in `server/src/server/world_server_enum.rs` today. §2n doesn't list G9 among the steps it explicitly says it supersedes (only G3c/G4/G5/G5b), but functionally the standalone "G9 design" never shipped separately — it shipped AS the WorldServer-enum work. |
