...
- **Phase 6 ✅ — borrow-builder de-zap (§2e first slice, Connor "proceed to (a) the queueing/coord-parameterization to kill the interim panics" 2026-06-29).** The 5a INTERIM panics on the Pipelined `WorldServer` arm are KILLED for all but one case. Mechanism, by residence (verified by trace + Explore agent):
  ...
  - **STILL panicking (the lone narrowed remainder):** per-USER `user_entity_priority(_mut)` — its layer is send-resident (`send.user_priorities`) AND borrow-returning (`EntityPriorityMut` holds `&mut HashMap`, can't escape a parked-slot `MutexGuard`). Clean fix = a symmetric coord-side `user_priority_mirror` + publish-on-send (mirrors the 4-E.2e global path at `world_server.rs:1103`); NOT built now because that publish is explicitly "future/unused" (`send_state_update.rs:26`), has scope-exit eviction complexity, and is net-new replication plumbing under-covered by the g9pre moat. Panic message narrowed to point at the cause + `global_entity_priority` alternative + §2e backlog. **This is the entire residual §2e backlog item.**
