...
3. Remove `EntityAndGlobalEntityConverter<E>` param from `write_updates` (entity-level converter is no longer needed in the writer; `world_entity` lookup moved to Phase 2 via `GlobalDiffHandler::world_entity`) — **DEFERRED**: requires `GlobalDiffHandler<E>` generification (see Phase 6 note); converter lookup in `write_updates` is now only O(entities_per_user) not O(N²), acceptable for current scale.
...
6. Remove `build_dirty_candidates_from_receivers` and `take_update_events` from `LocalWorldManager` — **DEFERRED**: both are CLIENT-PATH methods needed by `take_outgoing_events`. The server never calls them (`take_outgoing_commands` is the server entry point). Documented as client-only in LocalWorldManager.
