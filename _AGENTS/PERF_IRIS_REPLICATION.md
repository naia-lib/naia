...
3. Remove `EntityAndGlobalEntityConverter<E>` param from `write_updates` — **INTENTIONALLY KEPT**. The converter's only use in `write_updates` is one O(1) `global_entity_to_entity` lookup per entity — not an O(N²) problem. Removing it requires `GlobalDiffHandler<E>` generification (blocked by object-safe `GlobalWorldManagerType` trait) and creates a client-path gap (no other `world_entity` source when `global_diff_handler` is `None`). The O(N²) targets are fully achieved by steps 1–2; step 3 is cosmetic cleanup that creates more friction than value.
