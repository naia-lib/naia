...
| D5 | **Unified priority sort — NOT tier reservation.** All outbound candidates (entity bundles + per-channel message batches) compete in a single k-way-merged sort each tick, gated only by the bandwidth budget. | Anti-starvation by construction; eliminates tier-specific starvation paths; elegance; matches Unity/Unreal production designs. |
