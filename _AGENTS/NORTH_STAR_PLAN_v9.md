...
### 2.2 Explicit Identity vs. Derived Identity
...
### 2.3 Baseline vs. Candidate Certification
...
*   **Mechanism:** Coverage map (`Rule → [Scenario]`) generated by Engine.
...
*   **Mechanism:** `namako verify` in CI. `namako repeat` for flake detection.
...
│  │   │ • repeat           │─────────▶│ • Strictness / Ambiguity Check     │    │  │
│  │   └────────────────────┘          │ • Certification Logic              │    │  │
│  │             │                     └────────────────────────────────────┘    │  │
│  │             │                                       │                       │  │
│  │             │   ┌───────────────────────────────────┘                       │  │
...
Engine and adapters MUST use a shared reference implementation for NCCP-1 encoding, hash computation, and payload-hash construction.
*   This is provided by the `namako_hash` crate, governed by `hash_contract_version`.
*   If an adapter cannot use the shared crate, it MUST implement NCCP-1 exactly and pass a conformance test suite.
```
...
2.  **Steps:** Ordered list of steps, each containing:
    ...
    - `payload_hash`: Hash of the **execution payload** (CBOR map of `effective_kind`, `binding_id`, `captures`, `docstring`, `datatable`, `step_text`) under NCCP-1.

### 5.2 Resolved Plan Hash
`resolved_plan_hash = blake3_256( NCCP-1( ResolvedPlanWithoutHashFields ) )`
...
*   The plan MUST be fully self-contained and deterministic under NCCP-1 rules.
...
3.  Engine serializes `FeatureAstNorm` deterministically (NCCP-1).
...
Output ordering (lists, maps) must be deterministic. Certification must not flake based on map iteration order.
...
-   **Bindings:**
    ...
    -   `impl_hash`: Deterministic hash of the binding implementation (e.g., blake3 of normalized token stream). **MUST change when the binding function’s expanded implementation body changes** (drift signal).
...
```json
{
  "header": {
    "npap_version": 2,
    "hash_contract_version": "nccp-1+blake3-256",
    "engine_version": "...",
...
*   **Optional Recommended Fields** (if present, computed via NCCP-1 + blake3-256):
    *   `adapter_build_hash`
    *   `cargo_lock_hash`
    *   `rustc_version`
...
### 11.3 Flake Defense (`namako repeat`)
`namako repeat --n 20 --same-seed` runs the scenario 20 times and asserts the run report (excluding durations) is bit-for-bit identical every time.

### 11.4 Namako Canonical CBOR Profile (NCCP-1) (Normative)
1.  **Encoding:** All hashed artifacts MUST be encoded using **NCCP-1** (defined here) and then hashed over the resulting bytes.
2.  **No indefinite lengths:** Indefinite-length arrays/maps/strings are forbidden.
3.  **Shortest numeric encoding:** Integers MUST use the shortest CBOR integer encoding. Floats are forbidden in hashed artifacts.
4.  **Text normalization:** All text strings MUST be Unicode **NFC** normalized prior to encoding.
5.  **Map key type restriction:** Map keys MUST be **text strings only** (NFC normalized).
6.  **Map key ordering:** Maps MUST be encoded in deterministic key order: keys are ordered by the **lexicographic order of their canonical CBOR-encoded key bytes**.
7.  **No unknown fields:** Any artifact consumed by engine/adapter MUST be schema-validated and MUST reject unknown fields.
8.  **Missing vs null:** For hashed artifacts, optional fields MUST be present and set to `null` when absent (never “missing”), unless the schema says otherwise.

**Hash algorithm (Normative):**
*   `hash_fn = BLAKE3-256` (32-byte digest, lowercase hex).
*   `hash_contract_version = "nccp-1+blake3-256"`. Changing encoding rules or hash fn requires bumping `hash_contract_version`.

#### 11.4.1 Hash Field Exclusion Rule (Normative)
...
*   Same for any object containing its own hash value (e.g., `feature_ast_hash`, `step_registry_hash`, `bindings_used_hash`, or future `run_report_hash`).
*   Hashes are always computed on **canonical CBOR bytes** (per NCCP-1), never on JSON text.
...
### QA Tools
*   `namako repeat`: Check determinism.
...
*   Implement `impl_hash` generation (proc-macro in Rust).
...
4. **No flakes allowed.**
   - A slice is not “done” until `namako repeat` is stable (durations excluded).
...
### Step 6: Determinism Lock (Flake Extermination)
**Goal:** ensure the slice is stable and reproducible.

**Agent actions:**
- Run: `namako repeat --same-seed --n 20`
- If any mismatch occurs (excluding durations):
  - identify nondeterminism (ordering, RNG, clocks, data structure iteration)
  - fix until repeat is bit-identical

**Exit condition:**
- `namako repeat` passes consistently.