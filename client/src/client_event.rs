use naia_shared::{EventType, LocalObjectKey, LocalEntityKey, LocalComponentKey, StateType};

/// An Event that is be emitted by the Client, usually as a result of some
/// communication with the Server
#[derive(Debug)]
pub enum ClientEvent<T: EventType, U: StateType> {
    ...
    /// state passed to the Client on initialization
    Tick,
    /// Occurs when an State has been assigned to the local host as a Pawn,
    /// meaning it can receive Commands from the Client
    AssignPawn(LocalObjectKey),
    /// Occurs when a Pawn State has been unassigned from the local host, meaning it
    /// cannot receive Commands from this Client
    UnassignPawn(LocalObjectKey),
    /// Occurs when a Pawn State needs to be reset to local state
