...
impl<P: Protocolize> HandshakeManager<P> {
    pub fn new(send_interval: Duration, rtt_initial_estimate: Duration, jitter_initial_estimate: Duration) -> Self {
                    ...
                    if let Some(my_timestamp) = self.pre_connection_timestamp {
                        ...
                        if my_timestamp == payload_timestamp {
                            ...
                            if let Some(tick_manager) = tick_manager {
                                tick_manager.set_initial_tick(server_tick, self.rtt_initial_estimate, self.jitter_initial_estimate);
                            }
