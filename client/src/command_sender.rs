...
use naia_shared::{EntityType, ProtocolType, Ref, Replicate};

/// Handles outgoing Commands
#[derive(Debug)]
pub struct CommandSender<P: ProtocolType, K: EntityType> {
    queued_outgoing_command: HashMap<K, Ref<dyn Replicate<P>>>,
}

impl<P: ProtocolType, K: EntityType> CommandSender<P, K> {
    ...
    /// Gets the next queued Command to be transmitted
    pub fn pop_command(&mut self) -> Option<(K, Ref<dyn Replicate<P>>)> {
